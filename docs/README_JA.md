...
### 🌊 特殊機能バリアント
- **ストリーミング途切れ防止モード**: モデル名に `抗截断/` プレフィックスを追加
  - 例: `抗截断/gemini-2.5-pro`
  - RP-Hub と同様にツールの引数で本文を受け取り、通常のテキストとして返します。完全な空応答のみ設定回数まで再試行し、自動追記は行いません。
...
- `SESSION_AFFINITY_ENABLED`: セッションアフィニティを有効化（デフォルト: true）
